...
const labelSearch = function(container, options, optionsTribute) {
    if (typeof container == 'string') {
        // 如果是選擇器
        if (/^(?:#|\.)\w+/.test(container)) {
            container = document.querySelector('labelSearch');
        } else {
            // 否則當 ID 處理了
            container = document.getElementById('labelSearch');
        }
    }
    ...
    var defaultsTribute = {
        ...
        noMatchTemplate: function() {
            return '<li class="t-user-empty">沒有匹配的人</li>';
        },
        selectTemplate: function(item) {
            ...
            return `<hr class="t-user-tag" data-name="${origin.key}" data-id="${origin.value}" is-align="false">`;
        },
        menuItemTemplate: function(item) {
            ...
            return `<a href="javascript:" class="t-user-li">
                <img class="t-user-avatar" src="${origin.avatar}" alt="${origin.value}">
                <div class="t-user-info">
                    <span class="t-user-name">${strName}</span>
                    <div class="t-user-department">
                    ${origin.department}
                    </div>
                </div>
            </a>`;
    ...
    if (elePopOver == 'auto' || !elePopOver) {
        ...
        elePopOver.className = 't-user-popover';
    ...
    container.addEventListener('mouseover', function(event) {
        ...
        if (eleTarget && eleTarget.tagName == 'HR') {
            ...
            elePopOver.style.left = boundTarget.left + 'px';
            elePopOver.style.top = (boundTarget.top + 40) + 'px';
